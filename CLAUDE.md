...
- **titan007 全量 64 場完成 ✅**：OddsPapi 歷史僅 3–6 個月拿不到 2022，改用 titan007 凍結歷史補 2022 世界盃 64 場做離線回測校準。
  ...
  - **逐場明細新增「t24h→收盤」形狀欄**（2026-06-15，與「形狀（全軌跡）」並排）：只比 t24h 與 closing 兩錨點、重用 `trajectory.build_segments/build_summary`（把兩錨點塞兩相鄰 decision 槽當單一 segment）產**同格式**中文 tag（4 維同一套；大小球適配大/小）；任一錨點缺→「—」不 crash。價值＝兩欄並排可分辨「水互換是 t24h 前就發生 vs 決策後到收盤才發生」。純本機頁、零額度、不碰公開頁/selector/live（`_titan_records`/`render_titan_index` 路徑，公開頁 render_index/fixture/backtest 未動）。
  - **完整 tag → 讓方過盤率 彙總（2026-06-16，`_titan_tag_lean`/`_render_titan_lean`）**：4 桶（pin讓/pin大/sing讓/sing大，各 64、不合併）× **t24h→收盤完整 tag** 分組 → 過盤率。**固定視角**：讓分看**收盤讓方**（收盤線<0主讓/>0客讓）、大小看**大球方**；**收盤平手盤 line==0 排除另計**（不混進讓方過盤率分母）。**讓分 tag 用「讓/受」視角**（讓降受升…，讓方＝收盤讓方、與過盤率同基準；line==0→「平手盤無讓受」）——**僅本機 `_t24h_closing_tag`，`trajectory._build_tag` 公開頁主/客契約原樣不動**。每列 tag/n/過盤率，n<10 灰掉⚠️、口徑寫死（樣本薄·不可作 2026 下注依據·PUSH 不計分母半 0.5）。互動：點列→篩逐場明細+**精準高亮該盤該莊單格**（沿用 shape 彙總同一引擎、兩表互斥）；整區塊 `<details>` 可收疊（預設收起）。與 shape 彙總並存。實測 256 筆僅 4 個 tag 達 n≥10（全在大小桶）。
